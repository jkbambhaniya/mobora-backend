...
async function login(req, res) {
  try {
    ...
    const refreshToken = jwt.sign({ id: vendor.id }, JWT_REFRESH_SECRET, { expiresIn: '7d' });
...
async function getProfile(req, res) {
  try {
    ...
    return sendError(res, 'An internal server error occurred while retrieving profile.', {}, 500);
  }
}
...
async function logout(req, res) {
  try {
    ...
    return sendError(res, 'An internal server error occurred during sign-out.', {}, 500);
  }
}
...
async function refresh(req, res) {
  try {
    ...
    if (!encryptedRefreshToken) {
      return sendError(res, 'Refresh token missing or session expired.', {}, 401);
    }
    ...
    try {
      ...
      console.warn('[Vendor Auth] Failed to decrypt refresh token:', err.message);
      return sendError(res, 'Invalid refresh token cookie payload.', {}, 401);
    }
    ...
    try {
      ...
      console.warn('[Vendor Auth] Failed to verify refresh token JWT:', err.message);
      return sendError(res, 'Refresh token has expired or is invalid.', {}, 403);
    }
    ...
    return sendSuccess(res, 'Token refreshed successfully!', { token }, 200);

  } catch (error) {
    console.error('[Vendor Auth] Token refresh error:', error.message);
    return sendError(res, 'An internal server error occurred during token refresh.', {}, 500);
  }
}
...
async function updateProfile(req, res) {
  try {
    const { name, email, phone, shop_name, address, payment_methods, profile_img } = req.body;
    ...
    return sendError(res, 'An internal server error occurred while updating profile.', {}, 500);
  }
}
...
async function changePassword(req, res) {
  try {
    ...
    if (!isPasswordValid) {
      return sendError(res, 'Invalid current password.', { currentPassword: 'The current password you entered is incorrect.' }, 400);
    }
    ...
    return sendError(res, 'An internal server error occurred while changing password.', {}, 500);
  }
}
